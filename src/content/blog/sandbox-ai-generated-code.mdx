...
- **If you must fetch during the run, use an allowlist.** Route through a proxy that permits your registry and nothing else. This is meaningfully weaker, because a compromised package on the registry is now inside your boundary, but it's still a world apart from unrestricted egress.
...
Draw the boundary there, with untrusted execution on one side and everything else on the other, and several properties fall out for free:
...
**The test command itself.** It's specified by whoever submits the task, and it runs by design. If your users can supply an arbitrary test command, that string is an execution primitive, and it deserves the same suspicion as the generated code, and it's a reason to keep the sandbox boundary meaningful even for code you think you trust.
