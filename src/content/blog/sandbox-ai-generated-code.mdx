...
*Kiwi runs coding agents with this boundary built in: only your test command executes in the sandbox, with egress denied by default, so the model's code never holds your provider key. You can read [how the isolation model works](https://docs.runkiwi.dev/sandbox) or [try it free](https://app.runkiwi.dev).*
